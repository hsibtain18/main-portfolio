...
export default function WalletComponent() {
  ...
  // Unified data loading on subID change
  useEffect(() => {
    const load = async () => {
      if (subID) {
        await fetchPreferences(subID); // Load theme/currency
        await loadAllData(); // Load wallet coins + coin list
      }
    };
    load();
  }, [subID]);

  // Wallet entries built from store
  const walletEntries = WalletCoin.map((entry: any, index: number) => ({
    entryID: entry.entryID ?? `${entry.coin.id}-${index}`,
    userId: subID,
    qty: entry.qty,
    price: entry.purchasePrice,
  ...
  const deleteCoin = async (entryID: string) => {
    await apiDelete(`wallet/${subID}/${entryID}`, subID);
    const updated = WalletCoin.filter((entry: any, index: number) => {
      const id = entry.entryID ?? `${entry.coin.id}-${index}`;
      return id !== entryID;
    });
    setWalletCoin(updated);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        ...
        <Button variant="outline" onClick={reloadAllData} className="ml-2">
          Refresh Wallet
        </Button>
      </div>
